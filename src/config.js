...
function assertByteArray(value) {
  ...
  if (!value.every((v) => Number.isInteger(v) && v >= 0 && v <= 255)) {
    throw new Error('PRIVATE_KEY contains invalid byte values. Each item must be an integer between 0 and 255.')
  }
}
...
function parsePrivateKey(input) {
  ...
  if (/^0x[0-9a-fA-F]+$/.test(key)) {
    ...
    if (hex.length !== 128) {
      throw new Error(`PRIVATE_KEY hex must contain 64 bytes (128 hex chars). Received: ${hex.length} hex chars`)
    }
